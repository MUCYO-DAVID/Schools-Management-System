"use client"

// Remove duplicate useState import since it's already included in the React import below
import { Search, Plus, Edit, Trash2, Filter, MapPin, Users, Calendar } from "lucide-react"
import type { School as SchoolType } from "../types"
import { useLanguage } from "../page"
import SchoolModal from "../components/SchoolModal"
import DeleteConfirmModal from "../components/DeleteConfirmModal"
import React, { useEffect, useState } from 'react';

interface School {
  id: string; // Changed to string to match Date.now().toString() in handleSaveSchool
  name: string;
  location: string; // Added based on filter logic
  type: "Public" | "Private"; // Added based on filter logic
  level: "Primary" | "Secondary" | string; // Added based on filter logic
  // Add other school properties as per your database schema
}

interface SchoolsProps {
  // No longer needed as data is fetched internally
}

export default function Schools({}: SchoolsProps) {
  const { t } = useLanguage();
  const [schools, setSchools] = useState<School[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState("");
  const [filterType, setFilterType] = useState<"All" | "Public" | "Private">("All");
  const [filterLevel, setFilterLevel] = useState<"All" | "Primary" | "Secondary">("All");
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingSchool, setEditingSchool] = useState<School | null>(null);
  const [deletingSchool, setDeletingSchool] = useState<School | null>(null);

  useEffect(() => {
    const fetchSchools = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/schools'); // Your backend API endpoint
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data: School[] = await response.json();
        setSchools(data);
      } catch (err: any) {
        setError(err.message);
      } finally {
        setLoading(false);
      }
    };

    fetchSchools();
  }, []);

  if (loading) {
    return <div>Loading schools...</div>;
  }

  if (error) {
    return <div>Error: {error}</div>;
  }

  const filteredSchools = schools.filter((school) => {
    const matchesSearch =
      school.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      school.location.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesType = filterType === "All" || school.type === filterType;
    const matchesLevel = filterLevel === "All" || school.level.includes(filterLevel);

    return matchesSearch && matchesType && matchesLevel;
  });

  const handleAddSchool = () => {
    setEditingSchool(null);
    setIsModalOpen(true);
  };

  const handleEditSchool = (school: School) => {
    setEditingSchool(school);
    setIsModalOpen(true);
  };

  const handleDeleteSchool = (school: School) => {
    setDeletingSchool(school);
  };

  const confirmDelete = () => {
    if (deletingSchool) {
      // This part would ideally involve an API call to delete the school from the backend
      setSchools(schools.filter((s) => s.id !== deletingSchool.id));
      setDeletingSchool(null);
    }
  };

  const handleSaveSchool = (schoolData: Omit<School, "id">) => {
    if (editingSchool) {
      // This part would ideally involve an API call to update the school in the backend
      setSchools(schools.map((s) => (s.id === editingSchool.id ? { ...schoolData, id: editingSchool.id } : s)));
    } else {
      // This part would ideally involve an API call to add the school to the backend
      const newSchool: School = {
        ...schoolData,
        id: Date.now().toString(), // Temporary ID generation
      };
      setSchools([...schools, newSchool]);
    }
    setIsModalOpen(false);
  };

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-4">{t("schools")}</h1>
        <p className="text-gray-600">Manage and view all registered schools in Rwanda</p>
      </div>

      {/* Controls */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
        <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
          {/* Search */}
          <div className="relative flex-1 max-w-md">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
            <input
              type="text"
              placeholder={t("search")}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>

          {/* Filters */}
          <div className="flex gap-4 items-center">
            <div className="flex items-center gap-2">
              <Filter className="w-5 h-5 text-gray-400" />
              <select
                value={filterType}
                onChange={(e) => setFilterType(e.target.value as any)}
                className="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
                <option value="All">{t("all")}</option>
                <option value="Public">{t("public")}</option>
                <option value="Private">{t("private")}</option>
              </select>
            </div>

            <select
              value={filterLevel}
              onChange={(e) => setFilterLevel(e.target.value as any)}
              className="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
              <option value="All">{t("all")}</option>
              <option value="Primary">{t("primary")}</option>
              <option value="Secondary">{t("secondary")}</option>
            </select>

            <button
              onClick={handleAddSchool}
              className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors"
            >
              <Plus className="w-5 h-5" />
              {t("addSchool")}
            </button>
          </div>
        </div>
      </div>

      {/* Schools Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredSchools.map((school) => (
          <div
            key={school.id}
            className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow"
          >
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-xl font-semibold text-gray-900 line-clamp-2">{school.name}</h3>
                <div className="flex gap-2">
                  <button
                    onClick={() => handleEditSchool(school)}
                    className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                    title={t("edit")}
                  >
                    <Edit className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => handleDeleteSchool(school)}
                    className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                    title={t("delete")}
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              </div>
              <div className="text-gray-600 text-sm space-y-2">
                <p className="flex items-center gap-2">
                  <MapPin className="w-4 h-4 text-gray-500" />
                  {school.location}
                </p>
                <p className="flex items-center gap-2">
                  <Users className="w-4 h-4 text-gray-500" />
                  {school.type} - {school.level}
                </p>
                {/* Add more school details here as needed */}
              </div>
            </div>
          </div>
        ))}
      </div>

      {/* Modals */}
      <SchoolModal
        open={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        onSave={handleSaveSchool}
        initialData={editingSchool}
      />
      <DeleteConfirmModal
        isOpen={!!deletingSchool}
        onClose={() => setDeletingSchool(null)}
        onConfirm={confirmDelete}
        message={`Are you sure you want to delete ${deletingSchool?.name}?`}
      />
    </div>
  );
}
