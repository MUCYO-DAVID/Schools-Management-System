...
export default function TeacherPortal() {
  ...
  const [studentSuggestions, setStudentSuggestions] = useState<StudentSearchResult[]>([]);
  ...
  // Keep gradeSchoolId synced with teacher's school if present
  useEffect(() => {
    if (user?.school_id) setGradeSchoolId(user.school_id);
  }, [user?.school_id]);

  // Student name search (debounced)
  useEffect(() => {
    if (activeTab !== 'grades' || !isAuthenticated) return;
    const q = gradeStudentQuery.trim();
    if (q.length < 2) {
          ...
          <div className="flex border-b border-gray-200 overflow-x-auto">
            {['announcements', 'messages', 'documents', 'fees', 'grades', 'events'].map((tab) => (
              <button
                ...
                className={`px-6 py-3 text-sm font-medium whitespace-nowrap ${activeTab === tab ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'
                  }`}
              >
              ...
              <div className="border border-gray-200 rounded-lg p-4 bg-gradient-to-br from-blue-50 to-indigo-50">
                ...
                <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => {
                        setSelectedStudentLabel('');
                        setGradeStudentId('');
                        setGradeStudentQuery(e.target.value);
                      }}
                      placeholder="Type student name or email..."
                      className="border border-gray-300 rounded-md px-3 py-2 text-sm w-full"
                    />
                    {(isSearchingStudents || studentSuggestions.length > 0) && !selectedStudentLabel && (
                      <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-56 overflow-auto">
                        ...
                        {!isSearchingStudents && studentSuggestions.length === 0 && (
                          <div className="px-3 py-2 text-xs text-gray-500">No students found</div>
                        )}
                        {!isSearchingStudents && studentSuggestions.map((s) => {
                          ...
                          return (
                            <button
                              ...
                              key={s.id}
                              onClick={() => {
                                setGradeStudentId(String(s.id));
                                setSelectedStudentLabel(label);
                                setGradeStudentQuery(label);
                                setStudentSuggestions([]);
                              }}
                  ...
                  <select
                    ...
                    className="border border-gray-300 rounded-md px-3 py-2 text-sm"
                  >
                    <option value="">{user?.school_id ? 'School (locked)' : 'Select school'}</option>
                    {schools.map((s: any) => (
