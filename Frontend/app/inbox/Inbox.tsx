              ...
              {[
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl transition-all duration-300 relative ${
                    activeTab === tab.id 
                    ? 'bg-purple-600/10 text-purple-700 dark:text-purple-400 font-bold' 
                    ...
                    rooms.map((room) => (
                      <button
                        ...
                        className={`w-full p-4 flex gap-4 text-left transition-all relative ${
                          selectedRoom === room.id ? 'bg-purple-600/10' : 'hover:bg-slate-100 dark:hover:bg-white/5'
                        }`}
                      >
                        <div className="relative shrink-0">
                          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-purple-500/20 to-indigo-500/20 border border-slate-200 dark:border-white/10 flex items-center justify-center font-bold text-purple-700 dark:text-purple-300 overflow-hidden">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start mb-0.5">
                            <h4 className="text-sm font-bold truncate pr-2 text-slate-900 dark:text-slate-100">
                              {room.name || room.school_name}
                            </h4>
                            <span className="text-[10px] text-slate-500 dark:text-slate-400 shrink-0 mt-0.5">
                ...
                <div className="p-4 space-y-6">
                  ...
                  <form onSubmit={handleSearch} className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-100 dark:bg-black/40 border border-slate-200 dark:border-white/10 rounded-2xl pl-10 pr-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-purple-600/50 transition-all placeholder:text-slate-500 dark:placeholder:text-slate-600"
                    />
                  </form>
                    ...
                    {loading && (searchQuery.length > 0 ? searchResults : allUsers).length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {(searchQuery.length > 0 ? searchResults : allUsers).map((u) => (
                          <div 
                            key={u.id} 
                            className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-3xl p-4 flex items-center gap-4 border border-slate-200 dark:border-white/5 transition-all group"
                          >
                            ...
                            >
                              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-purple-500/20 to-indigo-500/20 border border-slate-200 dark:border-white/10 flex items-center justify-center font-bold text-xl text-purple-400 overflow-hidden shadow-lg group-hover:scale-105 transition-transform duration-300">
                                {u.avatar_url ? (
                ...
                <div className="p-4 space-y-4">
                  ...
                  {pendingRequests.length === 0 ? (
                    ...
                    pendingRequests.map((req) => (
                      <div key={req.request_id} className="bg-white/5 rounded-3xl p-4 border border-slate-200 dark:border-white/5 space-y-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-slate-900 dark:text-white">{req.first_name} {req.last_name}</p>
                            <p className="text-[10px] text-slate-600 dark:text-slate-400">{req.role}</p>
                          </div>
                        </div>
          ...
          {!selectedRoom ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-12 bg-gradient-to-b from-purple-900/10 via-transparent to-transparent">
              <div className="relative mb-8">
                ...
                <div className="absolute -top-4 -right-4 w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 flex items-center justify-center shadow-2xl rotate-12">
            ...
            <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative">
                    <button 
                      ...
                      className="p-2.5 rounded-2xl bg-white/5 border border-slate-200 dark:border-white/5 hover:bg-white/10 transition-all text-slate-400"
                    >
                      <MoreVertical className="w-5 h-5" />
                    </button>
                    {showChatMenu && (
                      <div className="absolute right-0 mt-2 w-48 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-2xl shadow-xl z-50 overflow-hidden text-sm">
                        <button 
                          onClick={() => {
                      ...
                      <div className={`max-w-[70%] group`}>
                        <div className={`
                          ...
                          ${isOwn 
                            ...
                            : 'bg-slate-100 dark:bg-white/5 text-slate-900 dark:text-slate-100 rounded-tl-sm border border-slate-200 dark:border-white/5'
                          }
                        `}>
                          {!isOwn && (
                          ...
                          {msg.message_type === 'image' && msg.attachment_url && (
                            <div className="mt-2 rounded-2xl overflow-hidden border border-slate-200 dark:border-white/10 shadow-lg">
                              <img 
                          ...
                          {msg.message_type === 'document' && msg.attachment_url && (
                            <a 
                              ...
                              className={`mt-2 flex items-center gap-3 p-3 rounded-2xl border transition-all ${
                                isOwn ? 'bg-white/10 border-white/20 hover:bg-white/20' : 'bg-black/20 border-slate-200 dark:border-white/5 hover:bg-black/30'
                              }`}
                            >
              ...
              <div className="p-6 bg-slate-50 dark:bg-black/20 border-t border-slate-200 dark:border-white/5">
                {/* File Preview */}
                {selectedFile && (
                  <div className="mb-4 p-3 bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl flex items-center justify-between animate-in fade-in slide-in-from-bottom-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-bold truncate text-slate-900 dark:text-slate-200">{selectedFile.name}</p>
                ...
                <div className="flex items-center gap-3 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-3xl p-2 pl-5 focus-within:border-purple-600/50 transition-all shadow-inner">
                  <input
                    type="file"
                    ref={fileInputRef}
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent border-none focus:ring-0 text-sm py-3 text-slate-900 dark:text-slate-100 placeholder:text-slate-500 dark:placeholder:text-slate-600"
                  />
                  <button 
