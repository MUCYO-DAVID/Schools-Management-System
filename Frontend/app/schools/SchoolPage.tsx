      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-8">
        <div className="w-full">
          ...
          <div className="relative mb-12 group">
            ...
            <div className="relative surface-card rounded-[2.5rem] border border-slate-200 dark:border-white/5 p-6 shadow-2xl">
              <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
                ...
                <div className="relative flex-1 w-full lg:max-w-xl">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-100 dark:bg-black/40 border border-slate-200 dark:border-white/5 rounded-2xl pl-14 pr-6 py-4 text-sm font-medium text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-600/50 transition-all placeholder:text-slate-500 dark:placeholder:text-slate-600"
                  />
                </div>

                {/* Filters */}
                <div className="flex flex-wrap gap-4 items-center w-full lg:w-auto">
                  <div className="flex items-center gap-3 bg-slate-100 dark:bg-black/40 border border-slate-200 dark:border-white/5 rounded-2xl px-4 py-2">
                    <Filter className="w-4 h-4 text-slate-500" />
                    <select
                      aria-label="Filter by Type"
                      value={filterType}
                      onChange={(e) => setFilterType(e.target.value as any)}
                      className="bg-transparent text-sm font-bold text-slate-700 dark:text-slate-300 focus:outline-none cursor-pointer pr-4"
                    >
                      <option value="All" className="bg-white dark:bg-slate-900">{t("all")}</option>
                      <option value="Public" className="bg-white dark:bg-slate-900">{t("public")}</option>
                      <option value="Private" className="bg-white dark:bg-slate-900">{t("private")}</option>
                    </select>
                  </div>

                  <div className="bg-slate-100 dark:bg-black/40 border border-slate-200 dark:border-white/5 rounded-2xl px-4 py-2">
                    <select
                    ...
                    >
                      <option value="All" className="bg-white dark:bg-slate-900">{t("all")}</option>
                      <option value="Primary" className="bg-white dark:bg-slate-900">{t("primary")}</option>
                      <option value="Secondary" className="bg-white dark:bg-slate-900">{t("secondary")}</option>
                      <option value="TVET" className="bg-white dark:bg-slate-900">TVET</option>
                    </select>
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredSchools.map((school) => (
              <div
                key={school.id}
                className="group surface-card rounded-3xl border border-slate-200 dark:border-white/5 overflow-hidden flex flex-col hover:border-blue-500/40 transition-all duration-500 shadow-2xl relative"
              >
                ...
                <div className="h-32 w-full relative overflow-hidden shrink-0">
                  ...
                  <div className="absolute bottom-2 left-4 flex gap-1.5 z-20">
                    <span
                      className={`px-2 py-0.5 rounded-md text-[7px] font-black uppercase tracking-[0.15em] backdrop-blur-md border border-white/20 shadow-sm text-white ${
                        school.type === "Public" 
                          ? "bg-emerald-600/90" 
                          : "bg-blue-600/90"
                      }`}
                    ...
                    <span className="px-2 py-0.5 rounded-md text-[7px] font-black uppercase tracking-[0.15em] bg-black/60 backdrop-blur-md text-white border border-white/20 shadow-sm">
                      {school.level}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-4 flex-1 flex flex-col">
                  <div className="mb-3">
                    <h3 className="text-lg font-black text-slate-900 dark:text-white line-clamp-1 group-hover:text-blue-500 transition-colors italic tracking-tight">
                      {school.name}
                    </h3>
                  ...
                  <div className="space-y-2 border-t border-slate-200 dark:border-white/5 pt-3 flex-1">
                    <div className="flex items-center gap-2.5 text-slate-700 dark:text-slate-300">
                      <div className="w-7 h-7 rounded-lg bg-slate-100 dark:bg-white/5 flex items-center justify-center border border-slate-200 dark:border-white/5 shrink-0">
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-200 dark:border-white/5 flex justify-between items-center">
                    ...
                    <div className="flex -space-x-1">
                      {[1, 2, 3].map(i => (
                        <div key={i} className="w-4 h-4 rounded-full border border-slate-200 dark:border-slate-900 bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-[6px] font-bold text-slate-600 dark:text-slate-400">
          ...
          {filteredSchools.length === 0 && (
            <div className="text-center py-24 surface-card rounded-[3rem] border border-dashed border-slate-300 dark:border-white/10">
